...
# 后台IP设置
export Ipv4_ipaddr="10.1.1.2"            # 修改openwrt后台地址(填0为关闭)
export Netmask_netm="255.255.255.0"         # IPv4 子网掩码（默认：255.255.255.0）(填0为不作修改)
export Op_name="iStoreOS"                # 修改主机名称为OpenWrt-123(填0为不作修改)
...
# 修改插件名字
grep -rl '"终端"' . | xargs -r sed -i 's?"终端"?"TTYD"?g' || true
grep -rl '"TTYD 终端"' . | xargs -r sed -i 's?"TTYD 终端"?"TTYD"?g' || true
grep -rl '"网络存储"' . | xargs -r sed -i 's?"网络存储"?"NAS"?g' || true
grep -rl '"实时流量监测"' . | xargs -r sed -i 's?"实时流量监测"?"流量"?g' || true
grep -rl '"KMS 服务器"' . | xargs -r sed -i 's?"KMS 服务器"?"KMS激活"?g' || true
grep -rl '"USB 打印服务器"' . | xargs -r sed -i 's?"USB 打印服务器"?"打印服务"?g' || true
grep -rl '"Web 管理"' . | xargs -r sed -i 's?"Web 管理"?"Web管理"?g' || true
grep -rl '"管理权"' . | xargs -r sed -i 's?"管理权"?"改密码"?g' || true
grep -rl '"带宽监控"' . | xargs -r sed -i 's?"带宽监控"?"监控"?g' || true
